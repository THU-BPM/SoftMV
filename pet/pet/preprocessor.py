...
class MLMPreprocessor(Preprocessor):
    def aug_get_input_features(self, example: InputExample, **kwargs) -> InputFeatures:
        (input_ids, token_type_ids), (input_ids_aug, token_type_ids_aug), language_sample = self.pvp.aug_encode(example)
        return self._get_input_features(example, input_ids, token_type_ids), self._get_input_features(example, input_ids_aug, token_type_ids_aug), language_sample
    
    def get_input_features(self, example: InputExample, **kwargs) -> InputFeatures:
        input_ids, token_type_ids = self.pvp.encode(example)
        return self._get_input_features(example, input_ids, token_type_ids)

    def _get_input_features(self, example, input_ids, token_type_ids, **kwargs) -> InputFeatures:
