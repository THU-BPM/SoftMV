...
class LMPVP(AtomPVP):
    """implementing the convertion between label words to logits"""
...
class PVP(LMPVP):
    """Implementing pattern string processing."""
    ...
    def _encode(self, parts_a, parts_b):
    ...
    def aug_encode(self, example: InputExample) -> Tuple[List[int], List[int]]:
        (parts_a, parts_b), (parts_a_aug, parts_b_aug), language_sample = self.aug_get_parts(example)
        return self._encode(parts_a, parts_b), self._encode(parts_a_aug, parts_b_aug), language_sample

    def encode(self, example: InputExample) -> Tuple[List[int], List[int]]:
        parts_a, parts_b = self.get_parts(example)
        return self._encode(parts_a, parts_b)
