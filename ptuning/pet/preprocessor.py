...
class MLMPreprocessor(Preprocessor):
    ...
    def pct_get_input_features(self, example: InputExample, **kwargs) -> InputFeatures:
        input_ids, token_type_ids, block_flag, language_sample = self.pvp.pct_encode(example)
        attention_mask = [1] * len(input_ids)
        padding_length = self.wrapper.config.max_seq_length - len(input_ids)
        if padding_length < 0:
            raise ValueError(
