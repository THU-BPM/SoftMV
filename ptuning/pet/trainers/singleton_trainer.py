...
from pet.modules.wrapper import TransformerModelWrapper, PromptTuningModelWrapper
...
def init_model(config: WrapperConfig, train_config: WrapperConfig):
    ...
    model = PromptTuningModelWrapper(config, train_config)
    return model
...
def pct_train_single_model(
    ...
    results_dict = {}
    model.model.to(config.device)
    global_step, tr_loss, best_score, best_model = model.pct_train(
        -1,
        data_iterator,
        eval_config,
        config.device,
...
def init_iter_module(model_config, train_config, eval_config, ddp_config, dataset):
    ...
    if not ddp_config.do_ddp:
        ...
        eval_batch_size = train_config.per_gpu_train_batch_size * eval_config.n_gpu
    else:
...
def pct_init_iter_module(model_config, train_config, eval_config, ddp_config, dataset, output_dir, dict_dir):
    wrapper = init_model(model_config, train_config)
    if not ddp_config.do_ddp:
        train_batch_size = train_config.per_gpu_train_batch_size * train_config.n_gpu
    ...
    data_iterator.pct_patternize_trn(output_dir)
    data_iterator.patternize_val()
    data_iterator.patternize_zs()
    return (data_iterator, wrapper)
        ...
        for iteration in range(repetitions):
            ...
            if do_eval:
                ...
                for _eval_lang, _eval_loader in test_data_iterator.zs_iters.items():
                    ...
                    avg_acc += _eval_result["scores"]["acc"]
...
def pct_train_model_per_pattern(
    model_config: WrapperConfig,
    train_config: TrainConfig,
    eval_config: EvalConfig,
    ddp_config: DDPConfig,
        ...
        for iteration in range(repetitions):
            ...
            if do_train:
                if ddp_config.do_ddp:
                    ...
                    data_iterator, _ = pct_init_iter_module(
                        model_config, train_config, eval_config, ddp_config, dataset, pattern_iter_output_dir, dict_dir
                    )
                    finish_train = mp.Event()
                    queue = mp.Queue()
                    ddp_config.world_size = ddp_config.num_nodes * ddp_config.num_ranks
                    ...
                    data_iterator, wrapper = pct_init_iter_module(
                        model_config, train_config, eval_config, ddp_config, dataset, pattern_iter_output_dir, dict_dir
                    )
                    wrapper.rank_idx = torch.cuda.current_device()
                    trained_results, best_score, best_model = pct_train_single_model(
                        wrapper,
                        data_iterator,
                        train_config,
                        eval_config,
