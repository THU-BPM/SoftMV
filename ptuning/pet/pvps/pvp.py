...
class PVP(LMPVP):
    ...
    def pct_encode(self, example: InputExample) -> Tuple[List[int], List[int]]:
        parts_a, parts_b, block_flag_a, block_flag_b, language_sample = self.pct_get_parts(example)
        input_ids, token_type_ids, block_flag = self._encode(
            parts_a, parts_b, block_flag_a, block_flag_b
        )
        return (input_ids, token_type_ids, block_flag, language_sample)
