...
from prompt.utils import InputFeatures, DictDataset, InputExample
from prompt.modules.configs import PLM_WRAPPER
...
class PatternizedIterator(object):
    def __init__(
        ...
        # self.cosda_rate = trial.suggest_float("cosda_rate", 0.1, 0.9, step=0.05)
        self.cosda_lang = []
    ...
    def patternize_trn(self):
        ...
        logger.info(f"Pattern language: {self.pattern_lang}")
        features, features_target = self._trn_generate_dataset(self.dataset.trn_egs.egs)
    ...
    def _trn_convert_examples_to_features(
        self, examples: List[InputExample]
    ) -> List[InputFeatures]:
        features = []
        features_target = []
        for (ex_index, example) in enumerate(examples):
            ...
            if self.cosda_rate > 0:
                ...
                aug_example = example
            input_features_target = self.preprocessor.trn_get_input_features(aug_example)
            features.append(input_features)
    ...
    def _trn_generate_dataset(self, data: List[InputExample]):
        features1, features2 = self._trn_convert_examples_to_features(data)
        feature_dict1 = {
            "input_ids": torch.tensor(
