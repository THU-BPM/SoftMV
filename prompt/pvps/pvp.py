...
from prompt.utils import InputExample, get_verbalization_ids
from .base import AtomPVP
...
class LMPVP(AtomPVP):
    def __init__(self, wrapper, pattern_id: int):
        ...
        self.language_list = ['en', 'zh', 'de', 'fr', 'ar', 'bg', 'el', 'es', 'hi', 'ru', 'sw', 'th', 'tr', 'ur', 'vi']

        self.mlm_logits_to_cls_logits_tensor = (
            self._build_mlm_logits_to_cls_logits_tensor()
        )
        self.mlm_logits_to_cls_logits_tensor_uni = (
            self._build_mlm_logits_to_cls_logits_tensor_uni()
        )

    def _build_mlm_logits_to_cls_logits_tensor_uni(self):
        label_list = self.wrapper.config.label_list
        res = []
        for lang in self.language_list:
            m2c_tensor = (
                torch.ones([len(label_list), self.max_num_verbalizers_lang(lang)], dtype=torch.long)
                * -1
            )
            for label_idx, label in enumerate(label_list):
                verbalizers = self.verbalize_lang(label, lang)
                for verbalizer_idx, verbalizer in enumerate(verbalizers):
                    ...
                    # logger.info(f"m2c_tensor: {m2c_tensor}")
            res.append(m2c_tensor)
    ...
    def convert_mlm_logits_to_cls_logits_uni(
        ...
        # logger.info(f"masked_logits: {masked_logits.shape}")
        cls_logits = torch.stack(
            [self._convert_single_mlm_logits_to_cls_logits_universal(ml) for ml in masked_logits]
        )
        return cls_logits
    ...
    def _convert_single_mlm_logits_to_cls_logits_universal(
        self, logits: torch.Tensor
    ) -> torch.Tensor:
        m2c_list = self.mlm_logits_to_cls_logits_tensor_uni
        # logger.info(f"m2c_list: {m2c_list}")
        res = None
        for m2c in m2c_list:
            m2c = m2c.to(logits.device)
            # logger.info(f"m2c: {m2c}")
            filler_len = torch.tensor(
                [len(self.verbalize(label)) for label in self.wrapper.config.label_list],
                dtype=torch.float,
            )
            ...
            cls_logits = cls_logits * (m2c > 0).float()
            cls_logits = cls_logits.sum(axis=1) / filler_len
            # logger.info(f"cls_logits: {cls_logits}")
            if res is None:
                res = cls_logits
            else:
                res = torch.add(res, cls_logits)
                # logger.info(f"##################res: {res}")
        return res/15
...
class PVP(LMPVP):
    ...
    @abstractmethod
    def trn_get_parts(self, example: InputExample) -> FilledPattern:
        pass

    def _encode(self, parts_a: List[Tuple[str, bool]], parts_b: List[Tuple[str, bool]], block_flag_a, block_flag_b):
        ...
        assert len(parts_a) == len(
            block_flag_a
        ), f"{parts_a} \n {block_flag_a}, {len(parts_a)} vs {len(block_flag_a)}."
    ...
    def encode(self, example: InputExample) -> Tuple[List[int], List[int]]:
        ...
        input_ids, token_type_ids, block_flag = self._encode(
            parts_a, parts_b, block_flag_a, block_flag_b
        )
        return (input_ids, token_type_ids, block_flag)

    def trn_encode(self, example: InputExample) -> Tuple[List[int], List[int]]:
        parts_a, parts_b, block_flag_a, block_flag_b = self.trn_get_parts(example)
        input_ids, token_type_ids, block_flag = self._encode(
            parts_a, parts_b, block_flag_a, block_flag_b
        )
        return (input_ids, token_type_ids, block_flag)
