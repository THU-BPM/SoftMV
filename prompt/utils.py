...
def save_predictions(path: str, wrapper, results: Dict):
    ...
    for idx, prediction_idx in zip(results["guids"], results["predictions"]):
        prediction = inv_label_map[prediction_idx]
        idx = idx.tolist() if isinstance(idx, np.ndarray) else idx
        predictions_with_idx.append({"guid": idx, "label": prediction})
...
# 从json文件中读取数据
def read_json_file(file_path="common_datasets/full/multinli_1.0_train.jsonl", file_path2="common_datasets/full/multinli.train.en.tsv"):
    file1 = open(file_path, "r", encoding="utf-8")
    file2 = open(file_path2, "w", encoding="utf-8")
    file2.write("premise\thypo\tlabel\n")
    for line in file1.readlines():
        data = json.loads(line)
        file2.write(data["sentence1"] + "\t" + data["sentence2"] + "\t" + data["gold_label"] + "\n")

if __name__ == "__main__":
    file1 = open("../common_datasets/dev/multinli_1.0_dev_matched.jsonl", "r", encoding="utf-8")
    file2 = open("../common_datasets/dev/dev_full.jsonl", "w", encoding="utf-8")
    for line in file1.readlines():
        data = json.loads(line)
        data2 = {"sentence1": data["sentence1"], "sentence2": data["sentence2"], "gold": data["gold_label"], "file_idx": data["pairID"], "lang": "en"}
