...
import numpy as np
from prompt.utils import InputFeatures, InputExample
from prompt.pvps.pvp import PVP
from prompt.pvps import PVPS
...
class MLMPreprocessor(Preprocessor):
    ...
    def trn_get_input_features(self, example: InputExample, **kwargs) -> InputFeatures:
        input_ids, token_type_ids, block_flag = self.pvp.trn_encode(example)
        attention_mask = [1] * len(input_ids)
        padding_length = self.wrapper.config.max_seq_length - len(input_ids)
        if padding_length < 0:
            raise ValueError(
